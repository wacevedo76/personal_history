...
def create_ph_filename(userdata: Dict) -> Dict:
    ...
    # Unpack values from user_data["user_data"]
    creation_time, epoch_time, firstname, lastname = userdata["user_data"].values()
...
def generate_ph_file(userdata: Dict, directory_path=config.PY_FILE_LOCATION):
    """Create file  
       Args: 
    ...
    """
    # convert epoch user hases createing time to datetime object and Date/Time string
    ph_file_creation_datetime_object = datetime.fromtimestamp(user_data["epoch_time"])
    ph_file_creation_time_string = ph_file_creation_datetime_object.strftime("%A, %B %d %Y, %I:%M %p")
    ...
    def _check_file_existance(filename: str, directory_path) -> bool:
        pass
    ...
    def _create_ph_file(filename: str, userdata: Dict, directory_path: str):
        ...
        data = {
            "profile": {
                ...
                "creation_time_str": user_data["creation_time"],
